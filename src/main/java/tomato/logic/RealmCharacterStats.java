...
public class RealmCharacterStats {
    ...
    public void decode(String pcStats) {
        ...
        if (readStat(bitArray, CharacterStatistics.SHOTS_FIRED.getId())) {
            shots_fired = reader.readCompressedInt();
        }
        if (readStat(bitArray, CharacterStatistics.HITS.getId())) {
            hits = reader.readCompressedInt();
        }
        if (readStat(bitArray, CharacterStatistics.ABILITY_USED.getId())) {
            ability_used = reader.readCompressedInt();
        }
        if (readStat(bitArray, CharacterStatistics.TILES_DISCOVERED.getId())) {
            tiles_discovered = reader.readCompressedInt();
        }
        if (readStat(bitArray, CharacterStatistics.TELEPORTS.getId())) {
            teleports = reader.readCompressedInt();
        }
        if (readStat(bitArray, CharacterStatistics.POTIONS_DRUNK.getId())) {
            potions_drunk = reader.readCompressedInt();
        }
        if (readStat(bitArray, CharacterStatistics.KILLS.getId())) {
            kills = reader.readCompressedInt();
        }
        if (readStat(bitArray, CharacterStatistics.ASSISTS.getId())) {
            assists = reader.readCompressedInt();
        }
        if (readStat(bitArray, CharacterStatistics.GOD_KILLS.getId())) {
            god_kills = reader.readCompressedInt();
        }
        if (readStat(bitArray, CharacterStatistics.ASSISTS_AGAINST_GODS.getId())) {
            assists_against_gods = reader.readCompressedInt();
        }
        if (readStat(bitArray, CharacterStatistics.CUBE_KILLS.getId())) {
            cube_kills = reader.readCompressedInt();
        }
        if (readStat(bitArray, CharacterStatistics.ORYX_KILLS.getId())) {
            oryx_kills = reader.readCompressedInt();
        }
        if (readStat(bitArray, CharacterStatistics.QUESTS_COMPLETED.getId())) {
            quests_completed = reader.readCompressedInt();
        }
        if (readStat(bitArray, CharacterStatistics.PIRATE_CAVE.getId())) {
            pirate_cave = reader.readCompressedInt();
        }
        if (readStat(bitArray, CharacterStatistics.UNDEAD_LAIR.getId())) {
            undead_lair = reader.readCompressedInt();
        }
        if (readStat(bitArray, CharacterStatistics.ABYSS_OF_DEMONS.getId())) {
            abyss_of_demons = reader.readCompressedInt();
        }
        if (readStat(bitArray, CharacterStatistics.SNAKE_PIT.getId())) {
            snake_pit = reader.readCompressedInt();
        }
        if (readStat(bitArray, CharacterStatistics.SPIDER_DEN.getId())) {
            spider_den = reader.readCompressedInt();
        }
        if (readStat(bitArray, CharacterStatistics.SPRITE_WORLD.getId())) {
            sprite_world = reader.readCompressedInt();
        }
        if (readStat(bitArray, CharacterStatistics.MINUTES_ACTIVE.getId())) {
            minutes_active = reader.readCompressedInt();
        }
        if (readStat(bitArray, CharacterStatistics.TOMB_OF_THE_ANCIENTS.getId())) {
            tomb_of_the_ancients = reader.readCompressedInt();
        }
        if (readStat(bitArray, CharacterStatistics.OCEAN_TRENCH.getId())) {
            ocean_trench = reader.readCompressedInt();
        }
        if (readStat(bitArray, CharacterStatistics.FORBIDDEN_JUNGLE.getId())) {
            forbidden_jungle = reader.readCompressedInt();
        }
        if (readStat(bitArray, CharacterStatistics.MANOR_OF_THE_IMMORTALS.getId())) {
            manor_of_the_immortals = reader.readCompressedInt();
        }
        if (readStat(bitArray, CharacterStatistics.FOREST_MAZE.getId())) {
            forest_maze = reader.readCompressedInt();
        }
        if (readStat(bitArray, CharacterStatistics.LAIR_OF_DRACONIS.getId())) {
            lair_of_draconis = reader.readCompressedInt();
        }
        if (readStat(bitArray, CharacterStatistics.CANDYLAND_HUNTING_GROUNDS.getId())) {
            candyland_hunting_grounds = reader.readCompressedInt();
        }
        if (readStat(bitArray, CharacterStatistics.HAUNTED_CEMETERY.getId())) {
            haunted_cemetery = reader.readCompressedInt();
        }
        if (readStat(bitArray, CharacterStatistics.CAVE_OF_THOUSAND_TREASURES.getId())) {
            cave_of_thousand_treasures = reader.readCompressedInt();
        }
        if (readStat(bitArray, CharacterStatistics.MAD_LAB.getId())) {
            mad_lab = reader.readCompressedInt();
        }
        if (readStat(bitArray, CharacterStatistics.DAVY_JONES_LOCKER.getId())) {
            davy_jones_locker = reader.readCompressedInt();
        }
        if (readStat(bitArray, CharacterStatistics.ICE_CAVE.getId())) {
            ice_cave = reader.readCompressedInt();
        }
        if (readStat(bitArray, CharacterStatistics.DEADWATER_DOCKS.getId())) {
            deadwater_docks = reader.readCompressedInt();
        }
        if (readStat(bitArray, CharacterStatistics.THE_CRAWLING_DEPTHS.getId())) {
            the_crawling_depths = reader.readCompressedInt();
        }
        if (readStat(bitArray, CharacterStatistics.WOODLAND_LABYRINTH.getId())) {
            woodland_labyrinth = reader.readCompressedInt();
        }
        if (readStat(bitArray, CharacterStatistics.BATTLE_FOR_THE_NEXUS.getId())) {
            battle_for_the_nexus = reader.readCompressedInt();
        }
        if (readStat(bitArray, CharacterStatistics.THE_SHATTERS.getId())) {
            the_shatters = reader.readCompressedInt();
        }
        if (readStat(bitArray, CharacterStatistics.BELLADONNAS_GARDEN.getId())) {
            belladonnas_garden = reader.readCompressedInt();
        }
        if (readStat(bitArray, CharacterStatistics.PUPPET_MASTERS_THEATRE.getId())) {
            puppet_masters_theatre = reader.readCompressedInt();
        }
        if (readStat(bitArray, CharacterStatistics.TOXIC_SEWERS.getId())) {
            toxic_sewers = reader.readCompressedInt();
        }
        if (readStat(bitArray, CharacterStatistics.THE_HIVE.getId())) {
            the_hive = reader.readCompressedInt();
        }
        if (readStat(bitArray, CharacterStatistics.MOUNTAIN_TEMPLE.getId())) {
            mountain_temple = reader.readCompressedInt();
        }
        if (readStat(bitArray, CharacterStatistics.THE_NEST.getId())) {
            the_nest = reader.readCompressedInt();
        }
        if (readStat(bitArray, CharacterStatistics.LOST_HALLS.getId())) {
            lost_halls = reader.readCompressedInt();
        }
        if (readStat(bitArray, CharacterStatistics.CULTIST_HIDEOUT.getId())) {
            cultist_hideout = reader.readCompressedInt();
        }
        if (readStat(bitArray, CharacterStatistics.THE_VOID.getId())) {
            the_void = reader.readCompressedInt();
        }
        if (readStat(bitArray, CharacterStatistics.PUPPET_MASTERS_ENCORE.getId())) {
            puppet_masters_encore = reader.readCompressedInt();
        }
        if (readStat(bitArray, CharacterStatistics.LAIR_OF_SHAITAN.getId())) {
            lair_of_shaitan = reader.readCompressedInt();
        }
        if (readStat(bitArray, CharacterStatistics.PARASITE_CHAMBERS.getId())) {
            parasite_chambers = reader.readCompressedInt();
        }
        if (readStat(bitArray, CharacterStatistics.MAGIC_WOODS.getId())) {
            magic_woods = reader.readCompressedInt();
        }
        if (readStat(bitArray, CharacterStatistics.CNIDARIAN_REEF.getId())) {
            cnidarian_reef = reader.readCompressedInt();
        }
        if (readStat(bitArray, CharacterStatistics.SECLUDED_THICKET.getId())) {
            secluded_thicket = reader.readCompressedInt();
        }
        if (readStat(bitArray, CharacterStatistics.CURSED_LIBRARY.getId())) {
            cursed_library = reader.readCompressedInt();
        }
        if (readStat(bitArray, CharacterStatistics.FUNGAL_CAVERN.getId())) {
            fungal_cavern = reader.readCompressedInt();
        }
        if (readStat(bitArray, CharacterStatistics.CRYSTAL_CAVERN.getId())) {
            crystal_cavern = reader.readCompressedInt();
        }
        if (readStat(bitArray, CharacterStatistics.ANCIENT_RUINS.getId())) {
            ancient_ruins = reader.readCompressedInt();
        }
        if (readStat(bitArray, CharacterStatistics.DUNGEON_TYPES_COMPLETED.getId())) {
            dungeon_types_completed = reader.readCompressedInt();
        }
        if (readStat(bitArray, CharacterStatistics.FORAX.getId())) {
            forax = reader.readCompressedInt();
        }
        if (readStat(bitArray, CharacterStatistics.HEROIC_ABYSS_OF_DEMONES.getId())) {
            heroic_abyss_of_demones = reader.readCompressedInt();
        }
        if (readStat(bitArray, CharacterStatistics.HEROIC_UNDEAD_LAIR.getId())) {
            heroic_undead_lair = reader.readCompressedInt();
        }
        if (readStat(bitArray, CharacterStatistics.HIGH_TECH_TERROR.getId())) {
            high_tech_terror = reader.readCompressedInt();
        }
        if (readStat(bitArray, CharacterStatistics.ICE_TOMB.getId())) {
            ice_tomb = reader.readCompressedInt();
        }
        if (readStat(bitArray, CharacterStatistics.KATALUND.getId())) {
            katalund = reader.readCompressedInt();
        }
        if (readStat(bitArray, CharacterStatistics.MAD_GOD_MAYHEM.getId())) {
            mad_god_mayhem = reader.readCompressedInt();
        }
        if (readStat(bitArray, CharacterStatistics.MALOGIA.getId())) {
            malogia = reader.readCompressedInt();
        }
        if (readStat(bitArray, CharacterStatistics.ORYXS_CASTLE.getId())) {
            oryxs_castle = reader.readCompressedInt();
        }
        if (readStat(bitArray, CharacterStatistics.ORYXS_CHAMBER.getId())) {
            oryxs_chamber = reader.readCompressedInt();
        }
        if (readStat(bitArray, CharacterStatistics.ORYXS_SANCTUARY.getId())) {
            oryxs_sanctuary = reader.readCompressedInt();
        }
        if (readStat(bitArray, CharacterStatistics.RAINBOW_ROAD.getId())) {
            rainbow_road = reader.readCompressedInt();
        }
        if (readStat(bitArray, CharacterStatistics.SANTAS_WORKSHOP.getId())) {
            santas_workshop = reader.readCompressedInt();
        }
        if (readStat(bitArray, CharacterStatistics.THE_MACHINE.getId())) {
            the_machine = reader.readCompressedInt();
        }
        if (readStat(bitArray, CharacterStatistics.UNTARIS.getId())) {
            untaris = reader.readCompressedInt();
        }
        if (readStat(bitArray, CharacterStatistics.WINE_CELLAR.getId())) {
            wine_cellar = reader.readCompressedInt();
        }
        if (readStat(bitArray, CharacterStatistics.PARTY_LEVEL_UPS.getId())) {
            party_level_ups = reader.readCompressedInt();
        }
        if (readStat(bitArray, CharacterStatistics.LESSER_GODS_KILLS.getId())) {
            lesser_gods_kills = reader.readCompressedInt();
        }
        if (readStat(bitArray, CharacterStatistics.ENCOUNTER_KILLS.getId())) {
            encounter_kills = reader.readCompressedInt();
        }
        if (readStat(bitArray, CharacterStatistics.HERO_KILLS.getId())) {
            hero_kills = reader.readCompressedInt();
        }
        if (readStat(bitArray, CharacterStatistics.CRITTER_KILLS.getId())) {
            critter_kills = reader.readCompressedInt();
        }
        if (readStat(bitArray, CharacterStatistics.BEAST_KILLS.getId())) {
            beast_kills = reader.readCompressedInt();
        }
        if (readStat(bitArray, CharacterStatistics.HUMANOID_KILLS.getId())) {
            humanoid_kills = reader.readCompressedInt();
        }
        if (readStat(bitArray, CharacterStatistics.UNDEAD_KILLS.getId())) {
            undead_kills = reader.readCompressedInt();
        }
        if (readStat(bitArray, CharacterStatistics.NATURE_KILLS.getId())) {
            nature_kills = reader.readCompressedInt();
        }
        if (readStat(bitArray, CharacterStatistics.CONSTRUCT_KILLS.getId())) {
            construct_kills = reader.readCompressedInt();
        }
        if (readStat(bitArray, CharacterStatistics.GROTESQUE_KILLS.getId())) {
            grotesque_kills = reader.readCompressedInt();
        }
        if (readStat(bitArray, CharacterStatistics.STRUCTURE_KILLS.getId())) {
            structure_kills = reader.readCompressedInt();
        }
        if (readStat(bitArray, CharacterStatistics.THE_THIRD_DIMENSION.getId())) {
            the_third_dimension = reader.readCompressedInt();
        }
        if (readStat(bitArray, CharacterStatistics.BEACHZONE.getId())) {
            beachzone = reader.readCompressedInt();
        }
        if (readStat(bitArray, CharacterStatistics.HIDDEN_INTERREGNUM.getId())) {
            hidden_interregnum = reader.readCompressedInt();
        }
        if (readStat(bitArray, CharacterStatistics.SULFUROUS_WETLANDS.getId())) {
            sulfurous_wetlands = reader.readCompressedInt();
        }
        if (readStat(bitArray, CharacterStatistics.KOGBOLD_STEAMWORKS.getId())) {
            kogbold_steamworks = reader.readCompressedInt();
        }
        if (readStat(bitArray, CharacterStatistics.MOONLIGHT_VILLAGE.getId())) {
