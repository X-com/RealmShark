...
public class LootGUI extends JPanel {
    ...
    private static int displayBagLootIcons(
        ...
        for (int i = 0; i < 8; i++) {
            ...
            // Per-slot enchant code, null when the slot carries no enchant.
            String enchantCode = null;
            if (enchants != null && i < enchants.length) {
                String code = enchants[i];
                if (!code.isEmpty() && !code.equals("AAIE_f_9__3__f8=")) {
                    enchantCode = code;
                }
            }

            if (enchantCode != null) {
                enchantText = ParseEnchants.parse(enchantCode);
            ...
            // Enchant pings match on the ids decoded from the raw code, not
            // on the display text.
            java.util.ArrayList<String> matchedEnchants =
                data.getMatchedEnchantPings(enchantCode);
