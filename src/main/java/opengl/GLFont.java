...
public class GLFont {
     ...
     */
    public GLFont(boolean antiAlias) {
     ...
     * @param antiAlias Wheter the font should be antialiased or not
     */
    public GLFont(int size, boolean antiAlias) {
     ...
     */
    public GLFont(InputStream in, int size) throws FontFormatException, IOException {
        this(in, size, true);
    }
     ...
     * @throws IOException         If font can't be read
     */
    public GLFont(InputStream in, int size, boolean antiAlias) throws FontFormatException, IOException {
...
//        image = operation.filter(image, null);
     ...
     */
    public void drawText(GLRenderer renderer, CharSequence text, float x, float y, GLColor c) {
     ...
     * @param y        Y coordinate of the text position
     */
    public void drawText(GLRenderer renderer, CharSequence text, float x, float y) {
        drawText(renderer, text, x, y, GLColor.WHITE);
    }
    ...
    public void drawHeroTexts(GLRenderer renderer, ArrayList<HeroLocations> vaHero) {
        int textHeight = getHeight("0");
        int halfH = textHeight / 2;

        texture.bind();
        renderer.begin();
        for(HeroLocations hero : vaHero) {
            CharSequence text = hero.getIndexString();
            ...
            for (int i = 0; i < text.length(); i++) {
                char ch = text.charAt(i);
                Glyph g = glyphs.get(ch);
                renderer.drawTextureRegion(texture, drawX - halfW, drawY - halfH, g.x, g.y, g.width, g.height, c);
                drawX += g.width;
            }
        }
        renderer.end();
